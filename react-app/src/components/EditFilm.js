...
//
    ClickConfirmDelete=async (key)=>{
        await getDelete(deleteFilmApi,key);
...
dialog=(Tag)=>{
    ...
    doRenderForPage=()=>{
        ...
        console.log('--sssss---',this.props.pageInfo.totalPage)
...
render() {
  const  dialogStyle={
   ...
   let myList= myDatasource.map(data=><tr  key={data.film_id}>
           <td><input onChange={(event) => this.handleChangeBox(event,this.state.checkBoxList)} value={data.film_id} type={'checkbox'}/></td>
           <td>{data.film_id}</td>
